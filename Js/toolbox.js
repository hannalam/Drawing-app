	...
	this.selectTool = function(toolName) {
		...
		for (var i = 0; i < this.tools.length; i++) {
			if (this.tools[i].name == toolName) {
				...
				select("#" + toolName + "sideBarItem").style("border", "3px solid purple");

				//if the tool has an options area. Populate it now.
				if (this.selectedTool.hasOwnProperty("populateOptions")) {
					this.selectedTool.populateOptions();
				}
				//if the tool has an options area. Populate it in stapeTool.js.
				if (this.selectedTool.hasOwnProperty("circleOptions")) {
					this.selectedTool.circleOptions();
				}
				//if the tool has an options area. Populate it in stampTools.js.
				if (this.selectedTool.hasOwnProperty("stampOptions")) {
					this.selectedTool.stampOptions();
				}
			}
		}
	};
